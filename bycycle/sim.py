...
def sim_filtered_brown_noise(T, Fs, filter_f_range, N):
    ...
    filter_f_range : 2-element array (lo,hi)
        frequency range of simulated data
        if None: do not filter
        if (x, None): only do a highpass filter
    N : int
        order of filter
    ...
    if filter_f_range is None:
        # Do not filter
        # Generate 1/f^2 noise
        brownN = sim_brown_noise(int(T * Fs))
        return brownN
    elif filter_f_range[1] is None:
        # Make filter order odd if necessary
        nyq = Fs / 2.
        if N % 2 == 0:
            warnings.warn('NOTE: Increased high-pass filter order by 1 in order to be odd')
            N += 1
        ...
        # High pass filter
        taps = signal.firwin(N, filter_f_range[0] / nyq, pass_zero=False)
        ...
        # Filter
        nyq = Fs / 2.
        taps = signal.firwin(N, np.array(filter_f_range) / nyq, pass_zero=False)
        brownNf = np.convolve(taps, brownN, 'same')
        return brownNf[N:-N]
...
def sim_oscillator(T, Fs, freq, rdsym=.5):
    """Simulate a stationary oscillator with a certain risedecay symmetry
    ...
    N_samples_cycle = int(np.round(Fs / freq))
    N_cycles = int(np.ceil(T * freq))
...
def sim_noisy_oscillator(T, Fs, freq, rdsym=.5, f_hipass_brown=2, SNR=1):
    """Simulate an oscillator with brown noise
